 ...
 *
 *   // network
 *   CPlatoPayNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.platopay.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CPlatoPayNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CPlatoPayNetwork(UniValue objIn)
    {
...
// // can be under: PlatoPayNetwork
// //   -- signature requirements : Key1(User)
// class CPlatoPayNetworkVariable : public CGovernanceObject
// {
// private:
