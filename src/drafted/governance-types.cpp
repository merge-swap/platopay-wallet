...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "PlatoPayNetwork") type = PlatoPayNetwork;
    if(strType == "PlatoPayNetworkVariable") type = PlatoPayNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == PlatoPayNetwork) s = "PlatoPayNetwork";
    if(type == PlatoPayNetworkVariable) s = "PlatoPayNetworkVariable";
