...
const char * const BITCOIN_CONF_FILENAME = "platopay.conf";
const char * const BITCOIN_PID_FILENAME = "platopayd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "platopay" is a composite category enabling all PlatoPay-related debug output
            if(ptrCategory->count(string("platopay"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PlatoPayCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PlatoPayCore
    // Mac: ~/Library/Application Support/PlatoPayCore
    // Unix: ~/.platopaycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PlatoPayCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PlatoPayCore";
#else
    // Unix
    return pathRet / ".platopaycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty platopay.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override platopay.conf
