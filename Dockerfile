...
RUN deps="alpine-sdk curl autoconf automake libtool boost-dev openssl-dev libevent-dev git" && \
  ...
  mkdir /wallet &&\
  cd /wallet && \
  git clone https://github.com/merge-swap/platopay-wallet.git . &&\   
  ./autogen.sh && \ 
  ./configure LDFLAGS=-L/opt/db/lib CPPFLAGS=-I/opt/db/include && \ 
  make install && \ 
  strip /usr/local/bin/platopayd &&\ 
  strip /usr/local/bin/platopay-cli &&\ 
  rm /usr/local/bin/platopay-tx &&\ 
  rm /usr/local/bin/test_platopay &&\ 
  adduser -D wallet && \ 
  apk del $deps && \
  rm -r /opt/db/docs /var/cache/apk/* /wallet /db-$DB_VERSION 

VOLUME ["/home/wallet/.platopaycore"]
EXPOSE 5520/tcp 
USER wallet 
CMD platopayd -printtoconsole